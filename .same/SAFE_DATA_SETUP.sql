...
) AS v(cost_centre_id, budget_year, pigas_vote_code, pigas_sub_item, description, category, original_amount, ytd_expenditure, committed_amount, is_active)
WHERE NOT EXISTS (
  SELECT 1 FROM budget_lines
  WHERE budget_lines.cost_centre_id = v.cost_centre_id
  AND budget_lines.pigas_vote_code = v.pigas_vote_code
  AND budget_lines.pigas_sub_item = v.pigas_sub_item
