...
✅ Integrates with PIGAS AAP budget system
...
8. **Budget Officer** - PIGAS sync & budget management
...
CREATE → SUBMIT → HOD → DEAN → BURSAR → REGISTRAR → VC
         ↓
      COMMIT → PURCHASE → RECEIVE → PAY → PIGAS UPDATE
```
...
│   │       ├── pigas/page.tsx          # PIGAS integration
...
9. **/dashboard/pigas** - PIGAS data import and synchronization
...
3. **Initial Data Import** (30 minutes)
   - Import cost centres
   - Load AAP budget lines from PIGAS
   - Create admin users
...
4. **User Training** (1-2 days)
   ...
   - Train Budget Officers on PIGAS sync
...
4. **Bursar** (Emmanuel Saliki) validates
   - Final budget check against PIGAS
...
6. **After goods received**
   ...
   - Marks as paid
   - Exports to PIGAS for ledger update
...
✅ Eliminates delayed endorsements
✅ Integrates seamlessly with PIGAS AAP budget
✅ Provides real-time spending visibility for all managers
