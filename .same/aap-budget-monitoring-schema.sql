...
CREATE OR REPLACE FUNCTION update_aap_header_total()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE aap_header
...
CREATE TRIGGER trg_aap_line_update_header_total
AFTER INSERT OR UPDATE OR DELETE ON aap_line
FOR EACH ROW
EXECUTE FUNCTION update_aap_header_total();

-- Trigger to validate budget before GE posting
CREATE OR REPLACE FUNCTION validate_budget_before_ge()
RETURNS TRIGGER AS $$
DECLARE
...
CREATE TRIGGER trg_ge_line_validate_budget
BEFORE INSERT OR UPDATE ON ge_line
FOR EACH ROW
EXECUTE FUNCTION validate_budget_before_ge();
