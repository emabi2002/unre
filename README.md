...
- ✅ **PIGAS Integration**: Seamless sync with AAP budget for real-time spending visibility
...
5. **PIGAS Integration** - Import/sync AAP budget and expenditure data
...
3. DEAN APPROVAL (If > K5,000)
   ↓
4. BUDGET CHECK (PIGAS integration)
   ↓
...
- `budget_lines` - AAP budget lines linked to PIGAS votes
...
**Process:**
1. Export budget and expenditure report from PIGAS
2. Navigate to Dashboard → PIGAS Sync
...
**Sample CSV:**
```csv
Cost Centre Code,PIGAS Vote Code,AAP Code,Description,Original Budget,YTD Expenditure,Budget Year
...
- Read-only access to PIGAS database
- Scheduled automatic sync (daily/weekly)
- Real-time budget queries

#### 3. **API Integration** (Future - Phase 3)

- RESTful API connection to PIGAS
- Bi-directional data sync
- Event-driven updates

### Budget Mapping

AAP Budget Lines in this system map to PIGAS vote codes:

```
GE System Budget Line ← → PIGAS Vote Code
...
6. Export for PIGAS ledger update
...
- **Budget Updates**: Regular PIGAS sync
...
**Issue**: Budget not syncing from PIGAS
...
- [x] Budget tracking
- [x] CSV/Excel PIGAS import
- [x] Basic reporting
- [x] Audit trail

### Phase 2 (Q2 2025)
- [ ] Direct PIGAS database integration
...
- Integrating with PIGAS AAP budget system
