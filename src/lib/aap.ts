 ...
 */
export async function getBudgetVersionsByYear(yearId: number): Promise<BudgetVersion[]> {
  const { data, error } = await supabase
    .from('budget_version')
    .select(`
      *,
...
/**
 * Get active budget version for a fiscal year
 */
export async function getActiveBudgetVersion(yearId: number): Promise<BudgetVersion | null> {
  const { data, error } = await supabase
    .from('budget_version')
    .select(`
      *,
 ...
 */
export async function createBudgetVersion(input: CreateBudgetVersionInput, userId: string): Promise<BudgetVersion> {
  const { data, error } = await supabase
    .from('budget_version')
    .insert({
      ...input,
...
/**
 * Activate budget version (deactivates others for same year)
 */
export async function activateBudgetVersion(budgetVersionId: number, yearId: number): Promise<BudgetVersion> {
  // First deactivate all versions for this year
  await supabase
    .from('budget_version')
...
export async function createBudgetLine(input: CreateBudgetLineInput): Promise<BudgetLine> {
  const { data, error } = await supabase
    ...
    .insert({
      ...input,
      fund_source: input.fund_source || 'GoPNG',
 ...
 * Update budget line
 */
export async function updateBudgetLine(budgetLineId: number, updates: Partial<CreateBudgetLineInput>): Promise<BudgetLine> {
  const { data, error } = await supabase
    .from('budget_line')
    .update(updates)
