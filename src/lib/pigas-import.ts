...
export interface PIGASBudgetLine {
...
export interface PIGASImportResult {
  success: boolean;
...
/**
 * Parse CSV or Excel file containing PIGAS budget data
 * Expected columns: Cost Centre Code, Cost Centre Name, Budget Line Code, Description, Original Amount, Actual Expenditure
 */
export async function parsePIGASFile(file: File): Promise<PIGASBudgetLine[]> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Parse rows
        const budgetLines: PIGASBudgetLine[] = [];
...
/**
 * Import PIGAS budget data into database
 */
export async function importPIGASBudget(budgetLines: PIGASBudgetLine[]): Promise<PIGASImportResult> {
  const result: PIGASImportResult = {
    success: false,
    importedCount: 0,
    updatedCount: 0,
    errorCount: 0,
      ...
      if (existingBudgetLine) {
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            pigas_actual_expenditure: line.actualExpenditure,
...
/**
 * Validate PIGAS file before import
 */
export function validatePIGASData(budgetLines: PIGASBudgetLine[]): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
...
/**
 * Generate PIGAS import template
 */
export function downloadPIGASTemplate() {
  const template = [
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'PIGAS Budget Import');
  ...
  XLSX.writeFile(workbook, 'PIGAS_Budget_Import_Template.xlsx');
}
